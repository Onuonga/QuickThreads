...
/*
 * A thread is basically a stack.
 * Allocate stack using a generic allocator,
 * then round as needed to meet hardware alignment requirements.
 */
typedef struct thread_t {
  qt_t *qt;		/* Pointer to thread of function... */
  void *stk;		/* Aligned address in `mblock'. */
  void *top;		/* Set top of stack if reuse. */
  void *mblock;		/* Per-thread blcok; rounded to make `stk'. */
...
t_alloc (void)
{
  ...
  t->mblock = malloc (ssz);
  t->stk = (void *)ROUND (((iaddr_t)t->mblock), QT_STKALIGN);
  if (!t->stk) {
    perror ("malloc");
    exit (1);
  }
...
/*
 * Note: should be "void *" for type compatability,
 * but the caller promises to never pay attention
 * to the return value.
 */
  static void
...
#define T_NULL	((qt_helper_t *)t_null)
...
#define	T_SPLAT	((qt_helper_t *)t_splat)
...
test01 (int n)
{
  ...
  while (n>0) {
    /* RETVALUSED */
#ifndef NDEF
    top = QT_SP (stk, QT_STKBASE);	(void)QT_ARGS (top, 0, 0, 0, 0);
    n -= 1;
#else
    top = QT_SP (stk, QT_STKBASE);	(void)QT_ARGS (top, 0, 0, 0, 0);
    top = QT_SP (stk, QT_STKBASE);	(void)QT_ARGS (top, 0, 0, 0, 0);
...
/*
 * Really "void *" but value is never used.
 */
  static void
...
test02 (int n)
{
  ...
  t = t_create (test02_aux1, 0, 0);
    QT_BLOCKI ((qt_helper_t *)test02_aux2, 0, 0, t->qt);
  t_free (t);
  t = t_create (test02_aux1, 0, 0);
    QT_BLOCKI ((qt_helper_t *)test02_aux2, 0, 0, t->qt);
  t_free (t);
...
/*
 * Really "void *", but return value is never used.
 */
  static void
...
test06 (int n)
{
  ...
  while (n>0) {
    /* RETVALUSED */
    (void)QT_ARGS (t->top, 0, 0, 0, test06_aux2);
    QT_BLOCKI ((qt_helper_t *)test06_aux3, 0, 0, t->qt);
#ifdef NDEF
    /* RETVALUSED */
    (void)QT_ARGS (t->top, 0, 0, 0, test06_aux2);
...
test07 (int n)
{
  ...
  for (i=0; i<TEST07_N-1; ++i) {
    /* RETVALUSED */
    (void)QT_ARGS (t[i]->top, 0, &t[i]->qt, &t[i+1]->qt, test07_aux2);
  }
  /* RETVALUSED */
  (void)QT_ARGS (t[i]->top, &n, &t[TEST07_N-1]->qt, &t[0]->qt, test07_aux3);
...
test08 (int n)
{
  ...
  for (i=0; i<TEST08_N-1; ++i) {
    /* RETVALUSED */
    (void)QT_ARGS (t[i]->top, 0, &t[i]->qt, &t[i+1]->qt, test08_aux2);
  }
  /* RETVALUSED */
  (void)QT_ARGS (t[i]->top, &n, &t[TEST08_N-1]->qt, &t[0]->qt, test08_aux3);
...
/*
 * Should be "void *", but return value is never used.
 */
  static void
test10_run (int arg0, ...)
{
...
#define TEST10_RUN	((qt_vuserf_t *)test10_run)
...
test10 (int n)
{
  ...
  while (--n >= 0) {
    test10_init (t, &main_thread, 0);
    QT_BLOCKI (T_SPLAT, &main_thread.qt, 0, t->qt);
  }
  t_free (t);
}
...
test11 (int n)
{
  ...
  while (--n >= 0) {
    test10_init (t, &main_thread, 2 * sizeof(int), 2, 1);
    QT_BLOCKI (T_SPLAT, &main_thread.qt, 0, t->qt);
  }
  t_free (t);
}
...
test12 (int n)
{
  ...
  while (--n >= 0) {
    test10_init (t, &main_thread, 4 * sizeof(int), 4, 3, 2, 1);
    QT_BLOCKI (T_SPLAT, &main_thread.qt, 0, t->qt);
  }
  t_free (t);
}
...
test13 (int n)
{
  ...
  while (--n >= 0) {
    test10_init (t, &main_thread, 8 * sizeof(int), 8, 7, 6, 5, 4, 3, 2, 1);
    QT_BLOCKI (T_SPLAT, &main_thread.qt, 0, t->qt);
  }
  t_free (t);
}
...
test14 (int n)
{
  ...
  while (--n >= 0) {
    test10_init (t, &main_thread, 0 * sizeof(int));
  }
  t_free (t);
}
