 ...
 *
 ...
 top + QT_STKBASE ->           STACK BOTTOM (higher address)
                               +==========================+
                               |                          |
                               +                          +
                                     ..............
                               ...
                               |                          |
                               +--------------------------+
 top + QT_ONLY_INDEX * 4 ->    | only param               | PAR(3)
                               +                          +
 top + QT_USER_INDEX * 4 ->    | userf param              | PAR(2)
                               +                          +
 top + QT_ARGT_INDEX * 4 ->    | t param                  | PAR(1)
                               +                          +
 top + QT_ARGU_INDEX * 4 ->    | u param                  | PAR(0)
                               +--------------------------+
 top + QT_RETURN_INDEX * 4 ->  | qt_start                 | LR save
                               +                          +
 top + QT_BLOCKI_FRAME_SIZE -> | top + QT_STKBASE         | backchain                     
                               +==========================+
                               |                          |
                               +                          +
                                     ..............
                               ...
                               |                          | 
                               +                          +
 top ->                        |top + QT_BLOCKI_FRAME_SIZE| backchain
                               +==========================+
                               STACK TOP (lower address)
 ...
 *
 ...
 bottom ->                     STACK BOTTOM (higher address)
                               ...
                               +                          +
                                     ..............
                               +                          +
 top + QT_VSTKBASE ->          | arg(0)                   | PAR(4)
                               +--------------------------+
 top + QT_CLEANUP_INDEX * 4 -> | cleanup param            | PAR(3)
                               +                          +
 top + QT_USER_INDEX * 4 ->    | userf param              | PAR(2)  
                               +                          +
 top + QT_VSTARTUP_INDEX * 4 ->| startup param            | PAR(1)
                               +                          +
 top + QT_ARGT_INDEX * 4 ->    | t param                  | PAR(0)
                               +--------------------------+
 top + QT_RETURN_INDEX * 4 ->  | qt_start                 | LR save
                               +                          +
 top + QT_BLOCKI_FRAME_SIZE -> | top + QT_STKBASE         | backchain                     
 ...
 */
#define QT_START_FRAME_SIZE PPC_ROUND_STACK(PPC_LINKAGE_AREA+PPC_PARAM_AREA(4))
 ...
 */
#define QT_VARGS_LOCAL_AREA (4*4)		/* local variable area */
 ...
 * 4+8 parameters. */
#define QT_VARGS_BKOFF PPC_PARAM_AREA(4+8)

#define QT_VSTART_FRAME_SIZE(varbytes) \
	PPC_ROUND_STACK(PPC_LINKAGE_AREA+QT_VARGS_BKOFF+(varbytes)+ \
		QT_VARGS_LOCAL_AREA)

/* Offset to the base of the varian argument list */
#define QT_VSTART_LIST_BASE (PPC_LINKAGE_AREA+PPC_PARAM_AREA(4))
 ...
 */
#define QT_RETURN_INDEX ((QT_BLOCKI_FRAME_SIZE+PPC_LR_SAVE)/sizeof(PPC_W))

/* static variable used to get the stack bottom in "VARGS" initialization */
static void *qt_sp_bottom_save;

#define QT_ARG_INDEX(i) ((QT_BLOCKI_FRAME_SIZE+PPC_PAR(i))/sizeof(PPC_W))
...
#define QT_GROW_DOWN
#define QT_STKALIGN	PPC_STACK_INCR
typedef PPC_W qt_word_t;


/* This macro is used by "QT_ARGS" to initialize a single argument thread.
...
#define QT_ARGS_MD(sp) \
	(QT_SPUT (sp, QT_RETURN_INDEX, qt_start), \
	QT_SPUT (sp, 0, sp+QT_BLOCKI_FRAME_SIZE), \
	QT_SPUT (sp, QT_BLOCKI_FRAME_SIZE/sizeof(PPC_W), \
		sp+QT_BLOCKI_FRAME_SIZE+QT_START_FRAME_SIZE))


/* This macro is used by "QT_VARGS" to initialize a variant argument thread. 
 ...
 * "qt_vstart". 
 */
#define QT_VARGS_MD0(sp, varbytes) \
  ((qt_sp_bottom_save = sp), \
  ((qt_t *)(((char *)(sp)) - \
		(QT_VSTART_FRAME_SIZE(varbytes)-QT_VSTART_LIST_BASE))))
...
#define QT_VARGS_MD1(sp) \
	(QT_SPUT (sp, QT_RETURN_INDEX, qt_vstart), \
	QT_SPUT (sp, 0, sp+QT_BLOCKI_FRAME_SIZE), \
	QT_SPUT (sp, (QT_BLOCKI_FRAME_SIZE)/sizeof(PPC_W), \
		qt_sp_bottom_save))
...
/* This macro is used by "QT_ADJ(sp)" to get the stack top form the stack
 ...
 */
#define QT_STKBASE \
	(QT_BLOCKI_FRAME_SIZE+QT_START_FRAME_SIZE)

/* This macro is used by "QT_VADJ(sp)" to get the stack top from the base
 * of the variant argument list during a variant argument thread initialization.
 */
#define QT_VSTKBASE	(QT_BLOCKI_FRAME_SIZE+QT_VSTART_LIST_BASE)

/* The *index* (positive offset) of where to put each value. */

#define QT_ARGU_INDEX	QT_ARG_INDEX(0)
#define QT_ARGT_INDEX	QT_ARG_INDEX(1)
#define QT_USER_INDEX	QT_ARG_INDEX(2)
#define QT_ONLY_INDEX	QT_ARG_INDEX(3)


#define QT_VARGT_INDEX		QT_ARG_INDEX(0)
#define QT_VSTARTUP_INDEX	QT_ARG_INDEX(1)
#define QT_VUSERF_INDEX		QT_ARG_INDEX(2)
#define QT_VCLEANUP_INDEX	QT_ARG_INDEX(3)

#endif /* ndef QT_POWERPC_H */
